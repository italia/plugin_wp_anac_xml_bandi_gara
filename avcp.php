...
add_action('admin_enqueue_scripts', function( $hook ) {
    ...
    if ( ('post-new.php' == $hook || 'post.php' == $hook) && 'avcp' === $post->post_type ) {
		wp_register_style( 'avcp_style',  plugins_url('italia-anac-xml-bandi-gara/includes/avcp_admin.css') );
        wp_enqueue_style( 'avcp_style');

        wp_enqueue_script( 'avcp_functions', plugins_url('italia-anac-xml-bandi-gara/includes/avcp_functions.js'), array( 'jquery' ) );
...
function avcp_activate() {
    $srcfile= ABSPATH . 'wp-content/plugins/italia-anac-xml-bandi-gara/includes/index.php.null';
...
function avcp_deactivate() {
    unlink(ABSPATH . 'italia-anac-xml-bandi-gara/index.php');
}
...
function avcp_get_basexmlurl($year = 0) {
    if(is_multisite()){
        ...
        if ( $year ) {
            return $upload_dir["baseurl"] . '/italia-anac-xml-bandi-gara/' .$year.'.xml';
        } else {
            return  $upload_dir["baseurl"] . '/italia-anac-xml-bandi-gara/';
        }
    }else{
        if ( $year ) {
            return get_site_url() . '/italia-anac-xml-bandi-gara/' .$year.'.xml';
        } else {
            return get_site_url() . '/italia-anac-xml-bandi-gara/';
        }
    }
...
function avcp_get_basexmlpath($year = 0) {
    if(is_multisite()){
        $upload_dir = wp_get_upload_dir();
        if ( $year ) {
            return $upload_dir["basedir"] . '/italia-anac-xml-bandi-gara/' . $year . '.xml';
        } else {
            return $upload_dir["basedir"] . '/italia-anac-xml-bandi-gara/';
        }
    }else{
        if ( $year ) {
            return ABSPATH . 'italia-anac-xml-bandi-gara/' . $year . '.xml';
        } else {
            return ABSPATH . 'italia-anac-xml-bandi-gara/';
        }
